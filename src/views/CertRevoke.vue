<template>
  <div class="cert-revoke">
    <h2>cert-revoke</h2>
    <p>
      This tool can be used to revoke Let's Encrypt certificates, even if you do not control the
      original ACME account that initially issued the certificate.
    </p>
    <p>You will need to be able to fulfill the following requirements:</p>
    <ul>
      <li>
        You control each domain that appeared on the certificate. This is because you will need to
        perform either the HTTP or DNS validation for each domain, to gain the ability to revoke
        previous certificates.
      </li>
      <li>
        You either already have the final certificate PEM you wish to revoke (which you can download
        from your web server or using your browser or openssl), or it can sometimes be found on
        crt.sh. You must provide the final certificate, a (poisoned) pre-certificate is not
        suitable.
      </li>
    </ul>
    <h3>Usage</h3>
    <div v-if="error" class="error">
      ...
      <p>
        Unfortunately something went wrong during the process. Usually this is not recoverable - you
    ...
    <div v-else-if="revoked">
      ...
      <p>
        Congratulations! The certificate revocation succeeded. You may verify this by checking the
        Let's Encrypt OCSP service in a while (it is heavily cached).
      </p>
      <div>
        <button @click="reset()" :disabled="loading" class="reset-button">Reset</button>
      </div>
    ...
    <div class="cert-selection" v-else-if="!certConfirmed">
      ...
      <div v-else-if="certSource === 'search'">
        <p>
          Please find your certificate on
          <router-link :to="{ name: 'cert-search', query: { d: 2160 } }" target="_blank"
            >cert-search</router-link
          >
          and copy the Certificate PEM contents before continuing (it begins with
      ...
      <div v-else>
        ...
        <div class="cert-display">
          ...
          <div class="cert-info">
            ...
            <div v-else>
              This certificate contains the following names. Ensure that you have the ability to
              either upload publicly accessible files or create DNS records for each of these
              domains before proceeding:
              <ul>
                <li v-for="name in certNames" v-bind:key="name">{{ name }}</li>
              </ul>
              <button @click="confirmCert()" :disabled="certNames.length === 0">
                OK, continue with this certificate
              </button>
            </div>
          </div>
        </div>
    ...
    <div v-else-if="certConfirmed">
      ...
      <p>
        Before the certificate can be revoked, you will need to perform authorizations for each of
        the names on the certificate.
      </p>
      <p v-if="loading">
        <em>Loading ... ({{ loadingTask }})</em>
      </p>
      <template v-for="authz in authzsToFulfill" v-bind:key="authz.url">
        <div class="authz" v-if="!loading && !authz.fulfilled">
          <em>
            You are required to complete an authorization for
            <strong>{{ authz.identifier.value }}</strong> in order to be able to revoke this
            certificate.
          </em>
          <p>
            Please follow the instructions below to complete this authorization. You only need to
            complete one of HTTP or DNS validation - both are not required.
          </p>
          <details open v-if="authz.httpKeyAuthz">
            ...
            <p>
              ...
              <em
                >http://{{ authz.identifier.value }}/.well-known/acme-challenge/{{
                  authz.httpResource
                }}</em
              >
              :
            ...
            <button
              @click="completeChallenge(authz, 'http-01')"
              :disabled="loading"
              v-if="!authz.fulfilled"
            >
              Complete HTTP Challenge
            </button>
          </details>
          <details open v-if="authz.dnsKeyAuthz">
            ...
            <button
              @click="completeChallenge(authz, 'dns-01')"
              :disabled="loading"
              v-if="!authz.fulfilled"
            >
      ...
      <button
        :disabled="loading"
        v-if="authzsRemaining === false && authzsToFulfill"
...
export default {
  ...
  methods: {
    ...
    confirmCert: async function () {
      ...
      try {
        ...
        const order = await this.acmeClient.newOrder(
          this.cert.getExtSubjectAltName2().map((v) => v[1])
        )

        this.loadingTask = 'Fetching authorizations'
        const authzs = await Promise.all(
          order.data.authorizations.map(async (v) => {
  ...
  computed: {
    ...
    authzsToFulfill: function () {
      ...
      return this.authzs.filter((v) => v.data.status === 'pending').map((v) => v.data)
    },
